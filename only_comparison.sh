...
grep "#" $pindvcf > $pindvcf0
awk 'BEGIN {OFS = "\t"} !/^#.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>=10 && ad[2]*100/(ad[1]+ad[2])<=15) print $0}' $pindvcf >> $pindvcf0

grep "#" $pindvcf > $pindvcf25
awk 'BEGIN {OFS = "\t"} !/^#.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>=10 && ad[2]*100/(ad[1]+ad[2])>15 && ad[2]*100/(ad[1]+ad[2])<=35) print $0}' $pindvcf >> $pindvcf25
#awk 'BEGIN {OFS = "\t"} !/^##.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>=ad[1]*0.15 && ad[2]<=ad[1]*0.35) print $1, $2, $3, $4, $5, $6, $7, $8}' $pindvcf >> $pindvcf25

grep "#" $pindvcf > $pindvcf50
awk 'BEGIN {OFS = "\t"} !/^#.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>=10 && ad[2]*100/(ad[1]+ad[2])>35 && ad[2]*100/(ad[1]+ad[2])<=65) print $0}' $pindvcf >> $pindvcf50
#awk 'BEGIN {OFS = "\t"} !/^##.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>ad[1]*0.35 && ad[2]<=ad[1]*0.65) print $1, $2, $3, $4, $5, $6, $7, $8}' $pindvcf >> $pindvcf50

grep "#" $pindvcf > $pindvcf100
awk 'BEGIN {OFS = "\t"} !/^#.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>=10 && ad[2]*100/(ad[1]+ad[2])>65) print $0}' $pindvcf >> $pindvcf100
#awk 'BEGIN {OFS = "\t"} !/^##.*/ {split($10,geno,":");split(geno[2],ad,","); if(ad[2]>ad[1]*0.65) print $1, $2, $3, $4, $5, $6, $7, $8}' $pindvcf >> $pindvcf100

# comp
pindvcf0gz=$pindvcf0.gz
pindvcf25gz=$pindvcf25.gz
pindvcf50gz=$pindvcf50.gz
pindvcf100gz=$pindvcf100.gz
...
tabix -f -p vcf $pindvcf0gz
tabix -f -p vcf $pindvcf25gz
tabix -f -p vcf $pindvcf50gz
tabix -f -p vcf $pindvcf100gz
...
tabix -f -p vcf $compindelgz

vspindvcf0=$RESULTS"/"$prefix"_pindel_filt0_minus_"${compindelname%.*}".vcf"
vspindvcf25=$RESULTS"/"$prefix"_pindel_filt25_minus_"${compindelname%.*}".vcf"
vspindvcf50=$RESULTS"/"$prefix"_pindel_filt50_minus_"${compindelname%.*}".vcf"
vspindvcf100=$RESULTS"/"$prefix"_pindel_filt100_minus_"${compindelname%.*}".vcf"

vcf-isec -f -c $pindvcf0gz $compindelgz > $vspindvcf0
vcf-isec -f -c $pindvcf25gz $compindelgz > $vspindvcf25
vcf-isec -f -c $pindvcf50gz $compindelgz > $vspindvcf50
vcf-isec -f -c $pindvcf100gz $compindelgz > $vspindvcf100
